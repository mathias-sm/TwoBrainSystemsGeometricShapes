...
def make_glm_report(model,
    ...
    all_components = [
        view_img(
            statistical_maps[name],
            bg_img=bg_img,
            threshold=norm.isf(alpha),
            title=name,
            draw_cross=False
            )
        for name in statistical_maps.keys()
    ]
    ...
    all_components_text = '<br>'.join([e.get_iframe() for e in all_components])
    ...
    report_text.width, report_text.height = "100%", "5000"
    return report_text
...
def _plot_contrasts(contrasts, design_matrices):
    ...
    for design_matrix in design_matrices:
        for contrast_name, contrast_data in contrasts.items():
            ...
            contrast_plot.figure.set_figwidth(10)
...
def make_stat_maps(model, contrasts):
    ...
    nilearn.glm.second_level.SecondLevelModel.compute_contrast
    """
    statistical_maps = {contrast_id: model.compute_contrast(first_level_contrast=contrast_val)
...
def _dmtx_to_svg_url(design_matrices):
    ...
    #    dmtx_title = 'Session/Run {}'.format(dmtx_count)
    ...
    #for dmtx_count, design_matrix in enumerate(design_matrices, start=1):
    #    dmtx_text_ = string.Template(dmtx_template_text)
    #    dmtx_title = 'Session/Run {} — movements'.format(dmtx_count)
    #    dmtx_plot, axs = plt.subplots(1, 3)
    #    dmtx_plot.suptitle(dmtx_title)
    #    for idx, ax in enumerate(["x", "y", "z"]):
    #        axs[idx].plot(design_matrix[f"trans_{ax}"], label=f"trans_{ax}")
    #        axs[idx].plot(design_matrix[f"rot_{ax}"], label=f"rot_{ax}")
    #        axs[idx].legend()
    #    url_design_matrix_svg = plot_to_svg(dmtx_plot)
    #    # prevents sphinx-gallery & jupyter from scraping & inserting plots
    #    plt.close()
