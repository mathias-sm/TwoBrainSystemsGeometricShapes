...
cp "../bids_dataset/derivatives/rsa/sub-average/figures/pop-${pop}_task-${task}_slice_model-"*.png $OUT
...
for model in "symbolic" "IT" "both"; do
  convert "pop-${pop}_task-${task}_slice_model-${model}.png" "pop-${pop}_task-${task}_slice_model-empty.png" \
...
convert "pop-${pop}_task-${task}_slice_model-empty.png" $to_merge -background none -layers flatten +repage "pre_closer.png"
...
cp output.png "pop-${pop}_task-${task}_slice_merged.png"

cp "pop-${pop}_task-${task}_slice_merged.png" "${START}/../bids_dataset/derivatives/rsa/sub-average/figures_stiched/"
